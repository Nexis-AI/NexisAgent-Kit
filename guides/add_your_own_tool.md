...
3. Add supporting functions in NexisAgentKit
...
### 3. Add Supporting Functions to NexisAgentKit
> `src/agent/index.ts`
```typescript:src/agent/index.ts
export class NexisAgentKit {
  // ... existing code ...
...
const customAction: Action = {
  ...
  handler: async (agent: NexisAgentKit, input: Record<string, any>) => {
...
// or alternatively
const result = await agent.customFunction("your-input"); // assuming you have implemented `customFunction` method in NexisAgentKit
console.log(result);
```
...
Add the supporting function to NexisAgentKit:
> `src/agent/index.ts`
```typescript:src/agent/index.ts
export class NexisAgentKit {
