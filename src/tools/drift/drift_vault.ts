...
import type { NexisAgentKit } from "../../agent";
...
async function getOrCreateVaultDepositor(agent: NexisAgentKit, vault: string) {
...
async function getVaultAvailableBalance(agent: NexisAgentKit, vault: string) {
  try {
...
*/
async function getIsOwned(agent: NexisAgentKit, vault: string) {
  try {
    const { vaultClient, cleanUp } = await initClients(agent);
    const vaultPublicKey = new PublicKey(vault);
    const vaultDetails = await vaultClient.getVault(vaultPublicKey);
 ...
 * @param name
 */
export async function getVaultAddress(agent: NexisAgentKit, name: string) {
