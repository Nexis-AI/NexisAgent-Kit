...
const sendTransactionWithPriorityFeeAction: Action = {
  ...
  description:
    "Sends SOL or NPL tokens from a wallet with an estimated priority fee, ensuring faster processing on the Solana network.",
  examples: [
    [
      {
        input: {
  ...
  schema: z.object({
    ...
    amount: z
      .number()
      .positive()
      .describe("Amount of SOL or NPL tokens to send."),
    to: z.string().describe("Recipient's PublicKey."),
    splmintAddress: z
      .string()
      .optional()
      .describe(
        "Optional NPL token address, if transferring tokens other than SOL.",
      ),
  }),
